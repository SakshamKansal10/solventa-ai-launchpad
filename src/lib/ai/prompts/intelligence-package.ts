...
const FLAT_FORMAT_RULE = `Return roadmapPhases and roadmapTasks as FLAT arrays, not nested inside each opportunity. Each opportunity has exactly one integer opportunityIndex (0, 1, or 2). Each phase in roadmapPhases carries the opportunityIndex it belongs to, plus its own phaseIndex (0, 1, 2... in order within that opportunity — exactly 3-4 phases per opportunity). Each task in roadmapTasks carries the opportunityIndex AND phaseIndex it belongs to, plus its own taskIndex (0, 1, 2... in order within that phase — exactly 2-3 tasks per phase). Every opportunityIndex/phaseIndex pair used in roadmapTasks must correspond to a real entry in roadmapPhases with the same opportunityIndex/phaseIndex.`;
...
3. roadmapPhases and roadmapTasks — flat arrays as described, giving each opportunity its own complete roadmap.`;
