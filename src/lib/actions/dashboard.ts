...
export const getDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const [profileRes, opportunitiesRes, dnaRes, activeRoadmapRes] = await Promise.all([
      ...
      // A deep link (e.g. from the "ideas ready" email) pins this to the
      // EXACT consultation it was generated for — a founder who ran a
      // second consultation before clicking an old email must still land on
      // the ideas that email actually described, not whatever is newest by
      // the time they click. Falls back to true-latest below if the pinned
      // id doesn't resolve (deleted, or simply absent for normal
      // navigation) rather than erroring the whole dashboard over it.
      consultationId
        ? supabase
            .from("business_dna")
    ...
    let dnaData = dnaRes.data;
    if (consultationId && !dnaData) {
    ...
    const genomeSource = dnaData?.normalized_signals
      ? (dnaData.normalized_signals as unknown as NormalizedProfile)
      : null;
    ...
    return {
      ...
      selected,
      saved,
      viewedConsultationId: latestBusinessDnaId,
      businessDna: dnaData
        ? {
            analysis: dnaData.founder_analysis as unknown as FounderDNA | FounderAnalysis | null,
            signals: dnaData.normalized_signals as unknown as NormalizedProfile,
          }
        : null,
